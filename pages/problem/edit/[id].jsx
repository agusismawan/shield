...
function ProblemEdit({ problem }) {
  ...
  return (
    <>
      <Layout key="LayoutProblemEdit">
        <Head>
          <title>Problem Edit</title>
        </Head>
        <section>
          <div className="py-6">
            <div className="max-w-full mx-auto px-4 sm:px-6 md:flex md:items-center md:justify-between md:space-x-5 lg:max-w-full lg:px-12">
              <h2>{problem.data.problemName}</h2>
              <button
                type="button"
...
export async function getServerSideProps(context) {
  const res = await fetch(
    `http://127.0.0.1:3030/v1/probman/problem/${context.params.id}`
  );
